...
config_sickbeard() {
	if [ -f /root/.sabnzbd/sabnzbd.ini ] && [ -f ${QPKG_DIR}/config.ini ]; then
		${CMD_SETCFG} SABnzbd sab_username `${CMD_GETCFG} misc username -f /root/.sabnzbd/sabnzbd.ini` -f ${QPKG_DIR}/config.ini
		${CMD_SETCFG} SABnzbd sab_password `${CMD_GETCFG} misc password -f /root/.sabnzbd/sabnzbd.ini` -f ${QPKG_DIR}/config.ini
		${CMD_SETCFG} SABnzbd sab_apikey `${CMD_GETCFG} misc api_key -f /root/.sabnzbd/sabnzbd.ini` -f ${QPKG_DIR}/config.ini
		
		${CMD_SETCFG} NZBMatrix nzbmatrix_username `${CMD_GETCFG} nzbmatrix username -f /root/.sabnzbd/sabnzbd.ini` -f ${QPKG_DIR}/config.ini
		${CMD_SETCFG} NZBMatrix nzbmatrix_apikey `${CMD_GETCFG} nzbmatrix apikey -f /root/.sabnzbd/sabnzbd.ini` -f ${QPKG_DIR}/config.ini
	fi
